...
def load_config():
    ...
    with open("/autograder/source/test-grader/config.json", "r") as file:
        ...
        required_config_vars = [
            ...
            "weightReturnedTests",
            "pomPath",
            "groupedDefaultTestsScore",
            "submitTestsScore",
            "timeToDeadline",
            "waitTimeAfterPreTest",
        ]
...
def compare_json(json1, json2, any_order):
    ...
    if isinstance(json1, list):
        if any_order:
            return all(
                any(compare_json(item1, item2, any_order) for item2 in json2)
                for item1 in json1
            ) and all(
                any(compare_json(item2, item1, any_order) for item1 in json1)
                for item2 in json2
            )
        return all(
            compare_json(item1, item2, any_order) for item1, item2 in zip(json1, json2)
        )
    return json1 == json2
...
def run_curl_test(test):
    ...
    if returncode != 0:
        return {
            "success": False,
            "reason": f"Error executing test '{test['name']}':\n{stderr}",
        }

    if response_code != expected_status:
        return {
            "success": False,
            "reason": f"Test '{test['name']}' failed: Expected status {expected_status}, got {response_code}",
        }
    ...
    if response_type == "json":
        try:
            ...
            return {
                "success": False,
                "reason": f"Test '{test['name']}' failed: Response body is not valid JSON",
            }
        ...
        if not compare_json(response_json, expected_json, any_order):
            return {
                "success": False,
                "reason": f"Test '{test['name']}' failed: Expected body {expected_json}, got {response_json}",
            }
        ...
        if response_body != expected_body:
            return {
                "success": False,
                "reason": f"Test '{test['name']}' failed: Expected body {expected_body}, got {response_body}",
            }
...
def run_junit_tests(test, setup, from_server):
    source, submission = (
        "/autograder/source/sample-submission",
    ...
    if setup:  # running default tests on sample server
            ...
            return [
                {
                    "name": test["name"],
                    "success": False,
                    "reason": f"Location '{test['location']}' does not match any test file.",
                }
            ]
        ...
        report_path = os.path.join(
            submission, "target/surefire-reports", f"TEST-{test['className']}.xml"
        )
        subprocess.run(
            ["mvn", "test", f"-Dtest={test_class_with_method}", "-f", pom_file_path]
        )
    if not os.path.exists(report_path):
        return [
            {"name": test["name"], "success": False, "reason": "Test report not found"}
        ]
            ...
            test_results.append(
                {
                    "name": full_test_name,
                    "success": False,
                    "reason": f"Test '{full_test_name}' Failed: {reason}",
                }
            )
        else:
            test_results.append(
                {
                    "name": full_test_name,
                    "success": True,
                    "reason": f"Test '{full_test_name}' Passed",
                    "className": classname,
                    "testMethodName": test_name,
                }
            )
    return test_results
...
def run_test(test, setup, from_server):
    try:
        ...
        return {
            "success": False,
            "reason": f"Error running test '{test['name']}'. It is likely the test is formatted incorrectly: {e}",
        }
    ...
    for test in tests:
        ...
        if test["type"] == "curl":
            results["results"].append(
                {"name": test["name"], "result": test_result, "test": test}
            )
            ...
            if setup or not from_server:
                test_path = os.path.join(
                    f"/autograder/{'source/sample-submission' if setup else 'submission'}",
                    test["location"],
                )
...
def upload_tests(assignment_title, student_id, tests, params):
    encoded_id = base64.b64encode(student_id.encode("utf-8")).decode(
        "utf-8"
    )  # just not in plaintext, but isn't sensitive anyway
...
def pre_test(submission_path):
    process = subprocess.Popen(
        ["bash", "/autograder/source/sample-submission/pre-test.sh"],
        cwd=submission_path,
...
def main():
    ...
    if len(tests) > 0:
        ...
        if err != "":
            write_output(
                {
                    "output": f"Error running pre-test script for sample submission:, please contact assignment administrators:\n{err}",
                    "tests": [],
                }
            )
            return
        ...
        if err != "":
            write_output(
                {
                    "output": f"Error running post-test script for sample submission:, please contact assignment administrators:\n{err}",
                    "tests": [],
                }
            )
            return
        ...
        feedback = [
            {
                ...
                if "description" in result["test"] and result["test"]["description"]
                else result["result"]["reason"],
                "visibility": "visible",
            }
        ...
        for result in sample_results["results"]:
            if (
                ...
                and result["test"]["name"] not in successful_test_names
            ):
    ...
    if not check_database_health():
        write_output(
            {
                "output": "Server is not running or not healthy. Please contact the assignment administrators. In the meantime, here are the outcomes of running your tests on THE SAMPLE SOLUTION.\n"
                + output_str,
                "tests": feedback,
            }
        )
    ...
    if response.status_code < 200 or response.status_code >= 300:
        write_output(
            {
                "output": f"Error uploading tests to the database. Please contact the assignment administrators. Response status {response.status_code}:\n{response.text}\nIn the meantime, here are the outcomes of running your tests on THE SAMPLE SOLUTION.\n{output_str}",
                "tests": feedback,
            }
        )
        return
    ...
    if not json_response["success"]:
        write_output(
            {
                "output": "Error uploading tests to the database. Please contact the assignment administrators. In the meantime, here are the outcomes of running your tests on THE SAMPLE SOLUTION.\n"
                + output_str,
                "tests": feedback,
            }
        )
    ...
    if err != "":
        write_output(
            {
                "output": f"Error running pre-test script for student submission, please contact assignment administrators:\n{err}\nIn the meantime, here are the outcomes of running your tests on THE SAMPLE SOLUTION.\n"
                + output_str,
                "tests": feedback,
            }
        )
    ...
    if err != "":
        write_output(
            {
                "output": f"Error running post-test script for student submission, please contact assignment administrators:\n{err}\nIn the meantime, here are the outcomes of running your tests on THE SAMPLE SOLUTION.\n"
                + output_str,
                "tests": feedback,
            }
        )
    ...
    feedback += [
        {
            ...
            if result["test"].get("isDefault", False)
            and "score" in result["test"]
            and result["result"]["success"]
            else 0,
            "max_score": result["test"]["score"]
            if result["test"].get("isDefault", False) and "score" in result["test"]
            else 0,
    ...
    upload_response = upload_results(
        assignment_title,
        student_id,
        [
            {"name": result["name"], "passed": result["result"]["success"]}
            for result in all_results["results"]
        ],
    )
...
def setup():
    ...
    try:
        with open(
            "/autograder/source/sample-submission/default-tests.json", "r"
        ) as file:
            tests = json.load(file)
    except:
        tests = []
    ...
    if not check_database_health():
        print(
            "Server is not running or not healthy. Please contact the database administrators. In the meantime, here are the outcomes of running your tests on THE SAMPLE SOLUTION.\n"
            + output_str
            + "\n"
            + test_response
        )
    ...
    if response.status_code < 200 or response.status_code >= 300:
        print(
            f"Error uploading tests to the database. Please contact the assignment administrators. Response status {response.status_code}:\n{response.text}\nIn the meantime, here are the outcomes of running your tests on THE SAMPLE SOLUTION.\n{output_str}\n{test_response}"
        )
        return
    json_response = response.json()
    if not json_response["success"]:
        print(
            "Error uploading tests to the database. Please contact the database administrators. In the meantime, here are the outcomes of running your tests on THE SAMPLE SOLUTION.\n"
            + output_str
            + "\n"
            + test_response
        )
